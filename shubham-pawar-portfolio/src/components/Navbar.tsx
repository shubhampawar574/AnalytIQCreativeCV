...
import { Sparkles, FileText, Bot, Menu, X, Trophy, Briefcase, GraduationCap, ChevronRight } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ onOpenResume, onOpenAI, activeSection }) => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-slate-950/85 backdrop-blur-md border-b border-emerald-500/20 shadow-lg shadow-black/50 py-3'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-2.5">
          <button
            onClick={onOpenAI}
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-semibold bg-gradient-to-r from-emerald-500/10 to-teal-500/10 hover:from-emerald-500/20 hover:to-teal-500/20 text-emerald-300 border border-emerald-500/30 transition-all hover:scale-105"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-emerald-500 hover:bg-emerald-400 text-slate-950 shadow-md shadow-emerald-500/20 transition-all hover:scale-105 active:scale-95"
          >
            <FileText className="w-4 h-4" />
            <span>CV Executive Summary</span>
          </button>
        </div>
        ...
        <div className="lg:hidden flex items-center gap-2">
          <button
            onClick={onOpenResume}
            className="p-2 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 sm:hidden"
            title="CV Summary"
          >
            <FileText className="w-4 h-4" />
          </button>
        ...
        <div className="lg:hidden bg-slate-950/95 backdrop-blur-xl border-b border-slate-800 px-4 py-5 shadow-2xl space-y-3">
          ...
          <div className="flex flex-col gap-2 pt-1">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-xs font-semibold bg-emerald-500 text-slate-950 font-bold"
            >
              <FileText className="w-4 h-4" />
