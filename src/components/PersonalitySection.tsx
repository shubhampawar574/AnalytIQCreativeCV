  ...
  return (
    <section id="personality" className="py-20 bg-slate-950 relative border-t border-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        {/* Executive Identity Card with Photo */}
        <div className="max-w-4xl mx-auto p-6 sm:p-8 rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl flex flex-col md:flex-row items-center gap-6">
          <div className="relative w-32 h-32 sm:w-36 sm:h-36 rounded-2xl overflow-hidden border-2 border-emerald-500/50 shadow-xl shrink-0">
            <img
              src={PERSONAL_INFO.photoUrl}
              alt="Shubham Pawar"
              className="w-full h-full object-cover object-[center_18%] scale-125"
          ...
          <div className="space-y-2 text-center md:text-left flex-1">
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
              <h3 className="text-xl sm:text-2xl font-black text-white">{PERSONAL_INFO.name}</h3>
              <span className="text-xs font-bold text-emerald-400 bg-emerald-950/60 border border-emerald-500/30 px-3 py-1 rounded-full">
                PGDM BA ’26 @ MDI Gurgaon
              </span>
            </div>
            <p className="text-xs sm:text-sm text-slate-300 font-medium">
              Tech + Business Analytics professional with demonstrated problem-solving, stakeholder management, cross-functional leadership, and athletic endurance.
            </p>
            <div className="flex flex-wrap gap-2 pt-2 justify-center md:justify-start">
              <span className="text-[11px] px-2.5 py-1 rounded-lg bg-slate-950 text-slate-300 border border-slate-800 font-semibold">
                Ex-SAP Solution Support Engineer
              </span>
              <span className="text-[11px] px-2.5 py-1 rounded-lg bg-slate-950 text-slate-300 border border-slate-800 font-semibold">
                MHT-CET 99.73 %ile Ranker
              </span>
              <span className="text-[11px] px-2.5 py-1 rounded-lg bg-slate-950 text-amber-300 border border-amber-500/30 font-semibold">
                SAP Cricket Tournament MVP
              </span>
            </div>
          </div>
        </div>
